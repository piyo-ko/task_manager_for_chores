...
select 
	...
	max(r.conducted_on) as last, 
	count(r.reg_chore_id) as num, 
	printf("%2.3f", (julianday(date(max(r.conducted_on))) - julianday(date(min(r.conducted_on))) + 1) / count(r.reg_chore_id)) as interval,
	r.reg_chore_id as ID,  
	i.what_to_do as task
from regular_chores_rec as r, regular_chores_idx as i
where r.reg_chore_id=i.reg_chore_id
...
select 
	min(r.conducted_on) as first, 
	count(r.reg_chore_id) as num, 
	(julianday('now') - julianday(date(min(r.conducted_on)))) / count(r.reg_chore_id) as interval,
	r.reg_chore_id as ID,  
	i.what_to_do as task
from regular_chores_rec as r, regular_chores_idx as i
where r.reg_chore_id=i.reg_chore_id
group by r.reg_chore_id having count(r.reg_chore_id) > 0
order by interval;
...
select 
	...
	max(r.conducted_on) as last, 
	count(r.reg_chore_id) as num, 
	(julianday(date(max(r.conducted_on))) - julianday(date(min(r.conducted_on)))) as interval,
	r.reg_chore_id as ID,  
	i.what_to_do as task
from regular_chores_rec as r, regular_chores_idx as i
where r.reg_chore_id=i.reg_chore_id
...
select min(conducted_on) as first, max(conducted_on) as last, count(*), reg_chore_id from regular_chores_rec group by reg_chore_id having count(*) > 1 order by reg_chore_id;

select min(conducted_on) as first, max(conducted_on) as last, count(*), reg_chore_id from regular_chores_rec group by reg_chore_id order by reg_chore_id;
*/
